function getCalibrationSum(strs: string[]): number {
  return strs.reduce((sum, str) => sum + calibrate(str), 0);
}

function calibrate(str: string): number {
  const firstDigitIndex = str.search(/[1-9]/);
  const reverseStr = str.split('').reverse().join('');
  const lastDigitIndex = reverseStr.search(/[1-9]/);

  return Number(str[firstDigitIndex]) * 10 + Number(reverseStr[lastDigitIndex]);
}
// console.log(calibrate('1abc2')); // 12
// console.log(calibrate('pqr3stu8vwx')); //38
// console.log(calibrate('a1b2c3d4e5f')); //15
// console.log(calibrate('treb7uchet')); // 77

const sum = getCalibrationSum([
